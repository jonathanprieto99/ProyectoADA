...
Matching min_matching_dynamic (vector<int>, vector<int>);
...
Matching opt_solution_dp(vector<Pair>, vector<Pair>);
Matching fill_matrix(vector<Pair>, vector<Pair>);
Matching getMatchGroup(vector<Pair>, vector<Pair>);
Matching getMatchDivision(vector<Pair>, vector<Pair>);
void print_matrix (vector<vector<Matching>>);

/* Matriz donde se almacenan soluciones a subproblemas */
vector<vector<Matching>> mem;
vector<vector<Matching>> mem_pg;
...
void print_matrix(vector<vector<Matching>> matrix)
{
    cout << "Matrix: ";
    for (int i = 0; i < matrix.size (); i++)
    {
        for (int j = 0; j < matrix[i].size (); j++) {
            cout << "(" << matrix[i][j].matching[i].i << "," << mem_pg[i][j].matching[i].j << ")" << " ";
            cout << endl << "Weight: " << matrix[i][j].weight << endl;
            cout << endl << "Weight: " << matrix[i][j].weight << endl;
        }
    }
}
...
Matching getMatchGroup(vector<Pair> A, vector<Pair> B)
{
    Matching min_match;
    Pair match;
    float sum = 0;
...
Matching getMatchDivision(vector<Pair> A, vector<Pair> B)
{
    Matching min_match;
    Pair match;
    float sum = 0;
...
Matching min_matching_dynamic(vector<int>A, vector<int>B)
{
    vector<Pair> result;
    ...
    mem_pg.resize (blocks_A.size ());
    for (int i = 0; i < blocks_A.size (); i++)
        mem_pg[i].resize (blocks_B.size ());

    Matching min_matching = fill_matrix(blocks_A, blocks_B);

    return min_matching;

}
Matching fill_matrix(vector<Pair> blocks_A, vector<Pair> blocks_B)
{
...
Matching opt_solution_dp(vector<Pair> A, vector<Pair> B)
{
    Matching min_match;
    int i = A.size () - 1;
    int j = B.size () - 1;
    ...
    for (int k = i-1; k >= 0; k--)
    {
        ...
        Matching min_left = mem_pg[left_A.size()-1][left_B.size()-1];
        Matching min_right =  getMatchGroup(right_A, right_B);
    ...
    for (int k = j-1 ; k >=0; k--)
    {
        ...
        Matching min_left = mem_pg[left_A.size()-1][left_B.size()-1];
        Matching min_right = getMatchDivision (right_A, right_B);
...
int main () {
    ...
    while (true) {
        ...
        cout << "[4] Matching Minimo Programacion Dinamica" << endl;
        cout << "[5] Ingresar Nuevo Vector" << endl;
        cout << "[0] Salir" << endl;
        while (!new_vector) {
